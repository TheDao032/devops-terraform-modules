locals {
  ...
  flattened_jenkins_secrets_parameters = flatten([
    for path, creds in local.jenkins_secrets_parameters : [
...
resource "random_password" "jenkins_secrets" {
  for_each = {
    for secret in local.flattened_jenkins_secrets_parameters :

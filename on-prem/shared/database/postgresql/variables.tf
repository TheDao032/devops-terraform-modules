...
# NOTE: the provider CONNECTION settings (password, sslmode, connect-database, superuser flag) are
# NOT module variables — the `provider "postgresql"` block is generated by Terragrunt's database.hcl
# from env vars (PG_ADMIN_PASSWORD, PG_SSLMODE, ...). The pg_* vars kept above (pg_host / pg_port /
# pg_superuser) are still used by sql.tf (docker exec psql -U) + outputs, so they stay.
